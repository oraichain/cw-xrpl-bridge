...
All tokens issued on XRPL that can be bridged from the XRPL to the Oraichain and back must have a representation on the
...
a `cosmos` prefix and encoding it into the hexadecimal currency notation used in XRPL. This will be used as a
...
corresponding [workflow](#send-from-xrpl-to-cosmos).
...
initiates [workflow](#send-from-cosmos-to-xrpl).
...
```text
cosmos_amount = xrpl_amount * 1e(token_decimals-15)
amount_after_bridging_fee = cosmos_amount - bridging_fee
received_amount = truncate_amount(cosmos_amount, ...)
```
...
![Send from XRPL to cosmos](./img/send-from-XRPL-to-cosmos.png)

### Send from Oraichain to XRPL

![Send from Oraichain to XRPL](./img/send-from-cosmos-to-XRPL.png)
